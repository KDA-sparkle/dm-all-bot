...
// Initialiser les variables en tant que tableaux, même si elles sont undefined
let excludedRoles = Array.isArray(config.excludedRoles)
  ? config.excludedRoles
  : [];
let excludedMembers = Array.isArray(config.excludedMembers)
  ? config.excludedMembers
  : [];

// Set pour garder une trace des utilisateurs déjà DM
...
client.on("messageCreate", async (message) => {
  if (
    ...
    message.content.startsWith("!dmall ")
  ) {
    ...
    if ((isDmAll && args.length < 1) || (!isDmAll && args.length < 2)) {
      message.channel.send(
        `Usage: \`${
          isDmAll ? "!dmall <message>" : "!dm <server_id> <message>"
        }\``
      );
      return;
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Are you sure you want to send the following message as a DM?\n\n**Message:**\n${msgToSend}\n\n*Note: The message will be sent without embed in the DMs.*`
      )
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "confirm") {
        await i.update({
          content: "Message confirmed. Sending...",
          components: [],
        });
        ...
        message.channel.send(
          `${message.author}, the messages have been successfully sent!`
        );
      } else if (i.customId === "cancel") {
        await i.update({
          content: "Sending canceled. Please rewrite your message.",
          components: [],
        });
      }
    });
    ...
    guilds.forEach((guild) => {
      if (count === 25) {
        // Maximum de 25 champs par embed
        embeds.push(embed);
        embed = new EmbedBuilder()
          .setTitle("Servers (cont.)")
          .setColor("#FF69B4"); // Nouveau embed si nécessaire
        count = 0;
      }
...
async function sendDM(member, msgToSend) {
  if (messagedUsers.has(member.user.id)) {
    console.log(
      `Message not sent to ${member.user.tag} (ID: ${member.user.id}) because they have already received a message.`
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `Message sent to ${member.user.tag} (ID: ${member.user.id}): ${msgToSend}`
    );
    messagedUsers.add(member.user.id); // Ajouter l'utilisateur au Set après envoi du message
    await new Promise((resolve) => setTimeout(resolve, 10000)); // Pause de 10 secondes entre chaque message
  } catch (error) {
    if (error.code === 50007) {
      console.error(
        `Cannot send message to ${member.user.tag} (DMs disabled or bot blocked).`
      );
    } else {
...
async function sendDMsToGuild(message, guild, msgToSend) {
  message.channel.send(
    `${message.author}, starting to send messages to members of the server ${guild.name}...`
  );
  try {
    ...
    message.channel.send(
      `${message.author}, the messages have been sent to the members of the server ${guild.name}.`
    );
  } catch (error) {
    console.error(error);
    message.channel.send(
      `${message.author}, an error occurred while sending the messages.`
    );
  }
}
...
async function sendDMsToAllGuilds(message, msgToSend) {
  message.channel.send(
    `${message.author}, starting to send messages to all members in all servers...`
  );
  try {
    ...
    message.channel.send(
      `${message.author}, the messages have been sent to all members in all servers.`
    );
  } catch (error) {
    console.error(error);
    message.channel.send(
